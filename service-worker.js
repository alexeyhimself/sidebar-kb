...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === 'openSidePanel_already_opened_tab') {
    ...
    if (!info.linkUrl)  // do not do anything if not hyperlink
      return;
    ...
    chrome.sidePanel.open({ windowId: tab.windowId }).then(() => {
      setTimeout(() => {
        ...
        chrome.runtime.sendMessage({ type: "context_menu_call_hyperlink", "link": info.linkUrl, "title": info.selectionText});
      }, 300);  // a bit wait because drag&drop events pass faster than the DOM update
    });
  }
});
